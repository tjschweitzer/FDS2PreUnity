...
class windODE:
    def __init__(self,directory, startpoints,fds_input_location, t_span):
        ...
        self.__timeList = np.sort(list([self.getFileTimeStep(file) for file in self.__qFiles]))
        self.__meshBounds = {}
        ...
        while lineCounter < len(lines):
            ...
            if "&MESH" in current_line:
                ...
                self.__meshBounds = {
                    ...
                    "J": float(IJK[1]),
                    "K": float(IJK[2])}
                self.voxalSize = {"X":(self.__meshBounds["X_MAX"] -  self.__meshBounds["X_MIN"])/self.__meshBounds["I"],
                                  "Y":(self.__meshBounds["Y_MAX"] -  self.__meshBounds["Y_MIN"])/self.__meshBounds["J"],
                                  "Z":(self.__meshBounds["Z_MAX"] -  self.__meshBounds["Z_MIN"])/self.__meshBounds["K"]}
                break
    ...
    def getStartingPoints(self):
        ...
        X_Min_Value = self.__meshBounds["X_MIN"] + self.voxalSize["X"] / 2.0  # Center point of mesh
        X_Max_Value = self.__meshBounds["X_MAX"] - self.voxalSize["X"] / 2.0  # Center point of mesh
        Y_Min_Value = self.__meshBounds["Y_MIN"] + self.voxalSize["Y"] / 2.0  # Center point of mesh
        Y_Max_Value = self.__meshBounds["Y_MAX"] + self.voxalSize["Y"] / 2.0  # Center point of mesh
        with open(self.fds_input_location) as f:
            lines = f.readlines()
        ...
        while lineCounter < len(lines):
            ...
            point = [(XB[0]+XB[1])/2.0,(XB[2]+XB[3])/2.0,(XB[5])+self.voxalSize["Z"]/2.0]
    ...
    def runODE(self):
        ...
        for t_start in self.__timeList:
            ...
            for startCounter in range(len(self.startpoints)):
                ...
                t_span = [min(self.__timeList[counter:]), max(self.__timeList[counter:])]
                ...
                if self.__maxVelocity < current_result_max_vel:
                    print(f"new max Velocity {current_result_max_vel} changed from {self.__maxVelocity}")
    ...
    def get_velocity(self,t, x):
        ...
        u_velocity = self.__memorizedFiles[currentFileName][index_values[0], index_values[1], index_values[2]][0]
        v_velocity = self.__memorizedFiles[currentFileName][index_values[0], index_values[1], index_values[2]][1]
        w_velocity = self.__memorizedFiles[currentFileName][index_values[0], index_values[1], index_values[2]][2]
...
def main(args):
    ...
    app =windODE(dir, ["X_MIN","X_MAX"],"/home/kl3pt0/Work/fds2Unity/trails.fds",  t_span)
